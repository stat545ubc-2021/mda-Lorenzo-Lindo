...
#### Exercise Option 1: Plotting the distribution of the mean radius of nuclei present in the sample images in this dataset (variable = radius_mean)
```
cancer_sample %>% 
...
#### Exercise Option 2: Calculating the expected area_mean of the nuclei present based on the observed radius_mean and assigning it as a new variable.  
```
cancer_sample %>% 
...
#### Exercise Option 4: Exploring the relationship between the radius_mean and the area_mean
```
cancer_sample %>% 
...
#### Exercise Option 6: A boxplot of the frequency of different observations of the area_mean in either "M" malignant or "B" benign observations
```
cancer_sample %>% 
  group_by(diagnosis) %>% 
