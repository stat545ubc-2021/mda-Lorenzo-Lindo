...
### Question 2
**Does the presence of a root barrier affect the diameter of trees?**
no_root_barrier <- sum(with(vancouver_trees, root_barrier == "N"))
has_root_barrier <- sum(with(vancouver_trees, root_barrier == "Y"))
num_observations <- nrow(vancouver_trees)

### Question 3
**Is there a difference in the frequency of tree species within a specific genus in a neighbourhood of interest? Within a specified neighbourhood (e.g. Riley Park), and within a given genus of interest (e.g. Prunus), are there any differences in the frequency among tree species?**

#### Summarizing: Based on two categorical variables, calculate two summary statistics of your choosing. 
The code below will calculate the mean frequency of each tree species within the "Acer" genus in the entire dataset.
```{r}
species_in_Acer <- vancouver_trees %>% filter(genus_name == "Acer") 

head(species_in_Acer)

```

### Question 4
**Are there any differences in the height range of all trees between neighbourhoods in Vancouver?**

#### Summarizing: Based on two categorical variables, calculate two summary statistics of your choosing
The code below will calculate the number of neighbourhoods represented in this dataset, then calculate the mean number of trees among neighbourhoods:
```{r}
unique_neighbourhoods <- vancouver_trees %>% group_by(neighbourhood_name) %>% summarize(count = n())
nrow(unique_neighbourhoods)
print(unique_neighbourhoods)

unique_neighbourhoods %>% summarize(mean_trees_per_neighbourhood = mean(count))
```
The code below will the mean range of the heights of the trees represented in this dataset:
```{r}
mean_height_ranges <- vancouver_trees %>% summarize(mean_height = mean(height_range_id))
print(mean_height_ranges)
```

Note that this code has limitations - it does NOT account for any misspelled neighbourhoods or alternative names for the same neighbourhood.

#### Graphing: Create a graph out of summarized variables that has at least two geom layers.
```{r}
unique_neighbourhoods %>% ggplot(aes(x = neighbourhood_name, y = count)) +
