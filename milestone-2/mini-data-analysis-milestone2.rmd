...
In order to untidy this dataset, I will widen the "assigned" variable.
...
# Untidy the trees_subset
untidy_trees_subset <- trees_subset %>% pivot_wider(names_from = assigned, values_from = assigned)
head(untidy_trees_subset)
dim(untidy_trees_subset)

```
As you can see, we have gone from 8 columns (variables) in trees_subset to 9 columns (variables) in untidy_trees_subset. This was because I pivoted wider and put the "Y" or "N" values for the "assigned" variable as its their variables, resulting in a net increase by 1 more clumn.

We can now re-tidy the data back to its original state.
```{r}
retidy_trees_subset <- untidy_trees_subset %>% pivot_longer(-c(tree_id:common_name), names_to = "assigned", values_to = "assigned", names_repair = "minimal", values_drop_na = TRUE)

head(retidy_trees_subset)
dim(retidy_trees_subset)
